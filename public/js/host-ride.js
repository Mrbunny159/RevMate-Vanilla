...
export async function hostRide() {
  try {
    ...
    // CRITICAL: Check both window.db and imported db
    if (!window.db) {
      console.error('❌ CRITICAL: window.db is not initialized!');
      console.log('🔍 Checking Firebase init status...');
      console.log('window.firebaseApp:', window.firebaseApp);
      console.log('window.auth:', window.auth);
      console.log('window.db:', window.db);
      showInlineMessage('Firestore not initialized (window.db missing). Please refresh page.', { type: 'error' });
      return;
    }

    console.log('✅ Firebase check passed, window.db exists:', window.db);
    ...
    console.log('🚀 Attempting to save ride to Firebase:', rideToSave);
    console.log('📝 Ride details:', {
      title: rideToSave.title,
      organizerId: rideToSave.organizerId,
      isPublic: rideToSave.isPublic,
      hasStartLocation: !!rideToSave.startLocation,
      hasEndLocation: !!rideToSave.endLocation
    });

    // Save via rides module helper
    const saveResult = await saveRide(rideToSave);
    
    console.log('💾 Save result:', saveResult);
    
    if (!saveResult || !saveResult.success) {
      console.error('❌ SAVE FAILED:', saveResult);
      showInlineMessage('Failed to save ride: ' + (saveResult?.error || 'Unknown error'), { type: 'error' });
      return;
    }
    
    console.log('✅ Ride saved successfully with ID:', saveResult.id);
