...
export async function saveRide(rideData = {}) {
  console.log('🔵 saveRide called with data:', rideData);

  try {
    // Validation 1: Check if db is available
    console.log('🔍 Checking db availability...');
    console.log('  - db from import:', db);
    console.log('  - window.db:', window.db);

    const dbToUse = db || window.db;

    if (!dbToUse) {
      const errorMsg = 'Firestore not initialized - both imported db and window.db are undefined';
    ...
    // Validation 2: Check ride data
    if (!rideData.title) {
      throw new Error('Ride title is required');
    }
    if (!rideData.organizerId) {
    ...
    console.log('✅ Validation passed, creating collection reference...');
    const colRef = collection(dbToUse, 'rides');
    ...
    console.log('📤 Sending to Firestore with payload:', {
      title: payload.title,
      organizerId: payload.organizerId,
      isPublic: payload.isPublic,
      participantsCount: payload.participants?.length,
      hasStartLocation: !!payload.startLocation,
      hasEndLocation: !!payload.endLocation
    });

    const res = await addDoc(colRef, payload);

    console.log('✅✅✅ RIDE SAVED SUCCESSFULLY! Document ID:', res.id);
    console.log('🎉 Ride is now in Firebase Firestore!');
